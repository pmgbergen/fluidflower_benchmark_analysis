...
class Bilbo:
    def __init__(
        ...
        self.color_correction = daria.ManualColorCorrection(roi=self.roi["color"])
        ...
        self._segment_geometry(update=True)
    ...
    def _segment_geometry(self, update: bool = False) -> None:
        ...
        # TODO make parameter - sell routine as typical workflow with parameters which need tuning.
        verbosity = True
        ...
        if Path("labels.npy").exists() and not update:
            ...
            # TODO make parameter - main goal: smooth out.
            median_radius = 40

            # Smooth the image to get rid of sand grains
            denoised = skimage.filters.rank.median(
                basis, skimage.morphology.disk(median_radius)
            )

            # TODO make parameter - to make the
            resize_factor = 0.25

            # Resize image
            denoised = skimage.util.img_as_ubyte(
                skimage.transform.rescale(denoised, resize_factor, anti_aliasing=False)
            )

            ##TODO make parameter - main goal: make TVD feasible
            # resize_factor = 0.25

            ## Resize image
            # resized = skimage.util.img_as_ubyte(
            #    skimage.transform.rescale(basis, resize_factor, anti_aliasing=False)
            # )

            ## Smooth the image to get rid of sand grains - apply TVD
            ...
            # TODO make parameter - the smaller the sharper. However, for finding continous regions, a larger value is suitable.
            gradient_radius = 10

            # Find continuous region, i.e., areas with low local gradient
            markers_basis = skimage.filters.rank.gradient(
                denoised, skimage.morphology.disk(gradient_radius)
            )

            # Turn on to better find a threshold value.
            if verbosity:
                plt.figure()
                plt.imshow(markers_basis)
                plt.show()

            # TODO make parameter - tuning paremeter
            threshold_value = 10

            markers = markers_basis < threshold_value
            markers = ndi.label(markers)[0]

            # TODO make parameter - the smalle the sharper the edge, which is intended here.
            edge_radius = 1

            # Find edges
            gradient = skimage.filters.rank.gradient(
                denoised, skimage.morphology.disk(edge_radius)
            )
            ...
            print("num labels", np.unique(labels).shape)
