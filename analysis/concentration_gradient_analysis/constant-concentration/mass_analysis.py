...
for i,dir in enumerate(seg_folders):
    ...
    for c, im in enumerate(list_dir[:-6]):
        ...
        mass_co2 = mass_analysis.free_co2_mass(seg_port1, pressure(t), 2)
        ...
        volume_dissolved = np.sum(mass_analysis.volume_map(seg_port1, 1))
    
        mass_co2_subregion1_vec.append(mass_analysis.free_co2_mass(seg_port1, pressure(t), roi = subregion1))
        mass_co2_subregion2_vec.append(mass_analysis.free_co2_mass(seg_port1, pressure(t), roi = subregion2))
    
        # Compute volume of dissolved CO2 in subregions
        volume_dissolved_subregion1 = np.sum(mass_analysis.volume_map(seg_port1, 1, roi = subregion1))
        volume_dissolved_subregion2 = np.sum(mass_analysis.volume_map(seg_port1, 1, roi = subregion2))
        
        # Compute volume of dissolved CO2 in seal in subregions
        esf_label = 3
        seg_port1_esf = np.zeros_like(seg_port1)
        seg_port1_esf[labels == esf_label] = seg_port1[labels == esf_label]

        volume_dissolved_esf_subregion1 = np.sum(mass_analysis.volume_map(seg_port1_esf, 1, roi = subregion1))
        volume_dissolved_esf_subregion2 = np.sum(mass_analysis.volume_map(seg_port1_esf, 1, roi = subregion2))
        
        # Compute mass of dissolved CO2 in seal
        volume_dissolved_esf = np.sum(mass_analysis.volume_map(seg_port1_esf, 1))
    ...
    df.to_excel(dir[-3:-1]+"_port1.xlsx",index=None)
