...
if readfromfile:
    ...
    for run in runs:
        ...
        appendix = "" if zero_swi else "swi_"
        rel_concentration_gradient_integral = np.load(
            f"results/{appendix}{run}_qty_5.npy"
        )
        rel_time = np.load(f"results/{appendix}{run}_time.npy")
    ...
    for run in runs:
        ...
        images = list(
            sorted(
                Path(
                    f"/media/jakub/Elements/Jakub/benchmark/data/large_rig/{run}"
                ).glob("*")
            )
        )[
            10:
        ...
        for i in range(num_concentrations):
            ...
            concentration = darsia.Image(
                np.load(concentrations[i]), width=2.8, height=1.5
            )
            ...
            if False:
                concentration_img = np.zeros(
                    (*concentration.img.shape[:2], 3), dtype=float
                )
                for i in range(3):
                ...
                cv2.drawContours(
                    concentration_img, contours_box_C, -1, (180, 180, 180), 3
                )
